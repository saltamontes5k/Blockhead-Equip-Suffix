...
namespace EquipSuffix
{
	...
	static void ParseINI(void)
	{
		...
		Log("========================================");
		Log(" BlockheadEquipSuffix  0.5a  initialising");
		Log("========================================");
		...
		Log("[INI] Loaded %zu race groups, %zu NPC groups, logging=%s",
			s_RaceGroups.size(), s_NPCGroups.size(), s_LogEnabled ? "ON" : "OFF");
	}
	...
	static void Clear(void)
	{
		...
		if (s_LogFile)
		{
			Log("BlockheadEquipSuffix 0.5a shutting down.");
			fclose(s_LogFile);
			s_LogFile = nullptr;
		}
	}
		...
		if (!applied)
		{
			...
			if (Race && raceName)
			{
				...
				if (group >= 0)
				{
					...
					Log("File check: %s  ->  %s", raceFullPath.c_str(),
						EquipSuffix::FileExists(raceFullPath.c_str()) ? "FOUND" : "not found");
...
extern "C"
{
	...
	bool OBSEPlugin_Load(const OBSEInterface* obse)
	{
		...
		EquipSuffix::Log("BlockheadEquipSuffix 0.5a loaded successfully.");
